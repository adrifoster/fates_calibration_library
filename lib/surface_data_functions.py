...
def get_surdat(surdat_file: str) -> xr.Dataset:
    ...
    surdat["PCT_NAT_PFT_UPDATED"] = (
        (surdat.PCT_NAT_PFT * (surdat.PCT_NATVEG / 100.0))
        / (surdat.PCT_NATVEG + surdat.PCT_CROP)
        * 100.0
    )
    surdat["PCT_CFT_UPDATED"] = (
        (surdat.PCT_CFT * (surdat.PCT_CROP / 100.0))
        / (surdat.PCT_NATVEG + surdat.PCT_CROP)
        * 100.0
    )
...
def get_pft_vars(pft_indices: list[int], cft_min: int = 15) -> tuple[str, str]:
    ...
    if all(pft < cft_min for pft in pft_indices):
        ...
        raise ValueError(
            "Mixed PFT types in pft_indices: contains both natural and crop PFTs."
        )


def get_dom_pft_landmask(
    surdat: xr.Dataset,
    lai: xr.DataArray,
    pft_indices: list[int],
    config: dict = None,
    dom_dat: xr.Dataset = None,
...
def get_lats_lons(
    ds: xr.Dataset, pft_indices: list[int], var
...
def extract_dominant_grids(
    surdat: xr.Dataset,
    lai: xr.Dataset,
    mapping_dict: dict,
    min_grid_cells: int = 20,
    config: dict = None,
) -> pd.DataFrame:
...
def extract_biome(
    biome: xr.Dataset, lats: list[float], lons: list[float], pfts: list[str]
) -> pd.DataFrame:
    ...
    obs_df = pd.DataFrame(
        {"lat": lats, "lon": lons, "pft": pfts, "biome": biome_extract}
    )
...
def create_mask_ds(
    original_surdat: xr.Dataset, grid_df: pd.DataFrame, pft_names: list[str]
) -> xr.Dataset:
    ...
    for i, point in enumerate(points):
        ...
        sub_surdat = original_surdat.where(
            original_surdat["lat"] == actual_point[0]
        ).where(original_surdat["lon"] == actual_point[1])
        sub_surdat["pft"] = xr.full_like(
            sub_surdat.LANDFRAC_PFT, pft_names.index(pfts[i]) + 1
        )
        sub_surdat = sub_surdat.where(original_surdat["lat"] == actual_point[0]).where(
            original_surdat["lon"] == actual_point[1]
        )
    ...
    surdat_out = surdat_out.drop_vars(
        ["LANDFRAC_PFT", "PCT_NAT_PFT", "PCT_CFT", "natpft", "cft"]
    )
...
def find_closest_point(
    point: tuple[float, float], lats: list[float], lons: list[float]
) -> tuple[float, float]:
...
def get_dom_surdat(
    surdat: xr.Dataset, dom_pfts: list[str], mapping_dict: dict
) -> xr.Dataset:
...
def extract_co_dominant_grids(
    surdat: xr.Dataset,
    lai: xr.DataArray,
    dom_pfts: list[str],
    mapping_dict: dict,
    dom_thresh: dict,
    min_grid_cells: int = 10,
    config: dict = None,
):
    ...
    nondom_pfts = [
        pft
        for pft, indices in mapping_dict.items()
        if pft not in dom_pfts
        and pft not in ["not_vegetated", "broadleaf_evergreen_arctic_shrub"]
        and indices
    ]
    ...
    for pft in nondom_pfts:
        ...
        num_gridcells, dompft = get_dom_pft_landmask(
            surdat, lai, pft_indices, config=config, dom_dat=dom_dat
        )
